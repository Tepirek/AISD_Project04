...
int contains(vector<worker*> listOfWorkers, vector<worker*> workersToCheck);
...
public:
        Graph(const vector<department*> listOfDepartments, const vector<taskGroup*> listOfTaskGroups) {
        ...
        void connectTasksToTaskGroups() {
                ...
                for (int i = 0; i < this->numberOfTasks; i++) {
                        for (int j = 0; j < this->numberOfTaskGroups; j++) {
                                for (int k = 0; k < (int) this->listOfTaskGroups[j]->listOfTasks.size(); k++) {
        ...
        void connectTaskGroupsToTarget() {
                for (int i = 0; i < this->numberOfTaskGroups; i++) {
                        int sumOfTasks = 0;
                        for (int j = 0; j < (int) this->listOfTaskGroups[i]->listOfTasks.size(); j++) {
        ...
        void printStatistics() {
                ...
                for (int i = 0; i < (int) this->listOfDepartments.size(); i++) {
                        printString(this->listOfDepartments[i]->name);
                        printf("\t%d\n", this->listOfDepartments[i]->numberOfWorkers);
                        for (int j = 0; j < (int) this->listOfDepartments[i]->listOfWorkers.size(); j++) {
                                printf("\tname: "); 
        ...
        int createCombinations() {
                int goal = 0;
                for (int i = 0; i < (int) this->listOfTaskGroups.size(); i++) {
                ...
                */
                vector<vector<worker*>> workerMatrix = getMatrixOfCombinations(this->listOfDepartments);
                for (int i = 0; i < (int) workerMatrix.size(); i++) {
                        tmpCombinationOfWorkers = workerMatrix[i];
                        /*
                        printf("\n");
                        printf("LOW => ");
                        for (int x = 0; x < tmpCombinationOfWorkers.size(); x++) {
                                printf("%d ", tmpCombinationOfWorkers[x]);
                        }
                        printf("\n");
                        */
                        tmpCost = 0;
                        workersToDelete = createDeleteWorkers(this->listOfWorkers, tmpCombinationOfWorkers);
                        this->createGraph(workersToDelete);
                        for (int j = 0; j < (int) tmpCombinationOfWorkers.size(); j++) {
                                tmpCost += tmpCombinationOfWorkers[j]->value;
                        }
                        maxFlow = this->calculateMaxFlow();
                        // printf("goal = %d, maxFlow = %d, tmpCost = %d, minCost = %d\n", goal, maxFlow, tmpCost, bestCost);
                        if (maxFlow == goal) {
                ...
                int ret = 0;
                for (int i = 0; i < (int) this->listOfDepartments.size(); i++) {
                        for (int j = 0; j < this->listOfDepartments[i]->numberOfWorkers; j++) {
                                ret = contains(this->listOfDepartments[i]->listOfWorkers, bestCombinationOfWorkers);
                        }
                        if (ret > 0) {
                                printString(this->listOfDepartments[i]->name);
                                printf(" %d\n", ret);
                                        ...
                                        if (residualCapacity && (predecessors[rowVertex] == -1)) {
                                                ...
                                                if (rowVertex == target) {
                                                        ...
                                                        while (parent != source) {
                                                                ...
                                                                this->graph[colVertex][parent]->flow += residualCapacities[target];
                                                                this->graph[parent][colVertex]->flow -= residualCapacities[target];
                                                                parent = colVertex;
                                                        }
                                                        escape = true; break;
                                                }
...
int main(void) {
        ...
        // tryb szeregowania i tryb wyjścia
        ...
        for (int i = 0; i < numberOfDepartments; i++) {
                ...
                // tworzenie pracowników dla każdej katedry
        ...
        for (int i = 0; i < numberOfGroups; i++) {
                ...
                // tworzenie zadań dla każdej grupy zadań
        ...
        if (schedulingMode == 1 && outputMode == 1) {
...
char* readString() {
        ...
        char c = '\0';
        int counter = 0;
        // czyszczenie buffora bo testy są wredne i mają dodatkowe spacje...
        // while ((c == getchar()) == ' ' || (c == getchar()) == '\n');
        while ((c = getchar()) == (char) ' ');
        while ((c = getchar()) != ' ') {
                if (c == '\n') continue;
...
vector<worker*> countWorkers(vector<department*> listOfDepartments) {
        vector<worker*> listOfWorkers;
        for (int i = 0; i < (int) listOfDepartments.size(); i++) {
                for (int j = 0; j < (int) listOfDepartments[i]->listOfWorkers.size(); j++) {
...
vector<int> countDays(vector<taskGroup*> listOfTaskGroups) {
        ...
        for (int i = 0; i < (int) listOfTaskGroups.size(); i++) {
                for (int j = 0; j < (int) listOfTaskGroups[i]->listOfTasks.size(); j++) {
...
vector<int> createDeleteWorkers(vector<worker*> listOfWorkers, vector<worker*> workersToCheck) {
        vector<int> toDelete;
        for (int i = 0; i < (int) listOfWorkers.size(); i++) {
                toDelete.push_back(0);
        }
        for (int i = 0; i < (int) listOfWorkers.size(); i++) {
                for (int j = 0; j < (int) workersToCheck.size(); j++) {
...
vector<vector<worker*>> getMatrixOfCombinations(vector<department*> listOfDepartments) {
        ...
        for (int k = 0; k < (int) listOfDepartments.size() - 1; k++) {
                for (int i = 0; i < listOfDepartments[k]->numberOfWorkers; i++) {
                        for (int j = 0; j < listOfDepartments[k + 1]->numberOfWorkers; j++) {
                                test.push_back(connectVectors(test[i], test[j + listOfDepartments[k]->numberOfWorkers]));
                        }
                }
        }
        /*
